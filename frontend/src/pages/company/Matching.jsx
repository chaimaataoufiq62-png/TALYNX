...
const CompanyMatching = () => {
  ...
  useEffect(() => {
    if (!selectedChallengeId) { setMatches([]); return; }
    setLoadingCandidates(true);
    // Use the live match endpoint
    api.get(`/company/challenges/${selectedChallengeId}/matches?eligibleOnly=true`)
      .then(res => {
        // Backend returns { challenge: {...}, matches: [...] }
  ...
  return (
    <div className="container page-wrapper">
      ...
      <div className="card" style={{ marginBottom: '2rem', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        {selectedChallengeId && (
          <span className="badge badge-primary">{matches.length} candidat{matches.length !== 1 ? 's' : ''} éligible{matches.length !== 1 ? 's' : ''}</span>
        )}
      </div>
      ...
      {!selectedChallengeId ? (
        ...
        <div className="page-loading">
          ...
          <p style={{ color: 'var(--texte-moyen)' }}>Calcul du matching en cours...</p>
        </div>
      ) : matches.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {matches.map((item, idx) => {
            // Can be nested { candidate: {...}, score } (live) or flat (saved)
            const c = item.candidate || item;
            const score = item.score;
            
            return (
              <div key={c.id || idx} className="card" style={{ display: 'flex', alignItems: 'flex-start', gap: '1.25rem', flexWrap: 'wrap', padding: '1.5rem' }}>
                {/* Rank + Avatar */}
                ...
                <div style={{ flex: 1, minWidth: '200px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', fontSize: '0.8rem', color: 'var(--texte-moyen)', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
                    {c.ecole && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                        <GraduationCap size={13} /> {c.ecole}
                      </span>
                    )}
                    {c.ville && (
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                        <MapPin size={13} /> {c.ville}
                      </span>
                    )}
                    {c.niveauEtude && (
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                    {(item.matchedSkills || []).map((skill, i) => (
                      <span key={i} className="skill-badge" style={{ background: 'rgba(22,163,74,0.1)', color: '#16A34A', border: '1px solid rgba(22,163,74,0.2)' }}>
                        {skill.competenceNom || skill.nom}
                      </span>
                    ))}
                    {(item.missingSkills || []).slice(0, 3).map((skill, i) => (
                      <span key={i} className="skill-badge" style={{ opacity: 0.6 }}>
                        {skill.competenceNom || skill.nom}
                      </span>
                    ))}
                  </div>
